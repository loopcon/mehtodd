...
class VistiorUserDatatable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->rawColumns(['action', 'status', 'usertype', 'professionalname']);
    }
    public function checkrights($row)
    {
        $menu = '';
        $editurl = route('user.visitor.edit', ['user' => $row->id]);
    ...
    public function query(User $model)
    {
        $role_id = $this->additionalData['role_id'] ?? 3;
        $models = User::select();
