...
class VideoController extends Controller
{
    ...
    public function ChangeStatusVideo(Request $request)
    {
        ...
        // Find the video and update its status
        $video = Video::find($request->row_id);
        $video->update(['status' => $request->value]);
        ...
        // Check if the video status is 1
        if ($request->value == 1) {
            ...
            addVideoNotification($title, $userIdsString, $professionId);
        }

        return response()->json(['status' => 200, 'message' => 'Video status changed successfully.'], 200);
    }
